...
class CharacterGraph:
    ...
    def generate_full_graph(self):
        ...
        for idx_current_node in tqdm(range(len(occurence_nodes_sort_by_pos)), desc='Graph creation'):
    ...
    def entity_graph_by_chapter(self):
        ...
        return [self.entity_interaction_graph(self.subgraph_from_chapter(chapter))
                for chapter in tqdm(chapter_nodes, desc="Novel progression")]
    ...
    def entity_interaction_graph(graph):
        ...
        for entity_1, entity_2 in tqdm(list(product(entity_nodes, entity_nodes)), desc='Graph creation'):
    ...
    def generate_and_save(self):
        """
        From the full_graph, will compute :
        - entity graph on the full novel
        - entity graph for each chapter
        - dynamic narrative graph of the novel

        Will dump as pickle file + export in gexf format the global entity graph and the dynamic graph in
        the data/graph/ folder
        """
        ...
        pickle.dump(dynamic_graph, open('data/graph/' + self.book_name + '-dynamic-graph.pkl', 'wb'))
        export_dynamic_graph(dynamic_graph, path='data/graph/', name=self.book_name + "-dynamic-graph")

        return entity_graph, dynamic_graph, entity_chapter_graph
